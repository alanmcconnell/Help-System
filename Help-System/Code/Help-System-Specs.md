...
| NFR-06 | All confirmation dialogs shall use `acm_SecurePopUp()` from `acm_Prompts.js` — no `alert()` or `confirm()`. `acm_SecurePopUp()` is a self-contained, Promise-based modal dialog requiring no external libraries. The `ACM_APP_TITLE` constant at the top of `acm_Prompts.js` must be set to the app's name. |
...
| Client | `acm_Prompts.js` | Self-contained modal dialog library. Provides `acm_SecurePopUp()`. Set `ACM_APP_TITLE` constant to the app name. No external dependencies. |
