...
- **Confirmation dialogs** — `acm_SecurePopUp()` from `acm_Prompts.js`; self-contained Promise-based modal, no external libraries required. Set `ACM_APP_TITLE` in the file to match the app name.
...
- **Client**: Quill 1.3.7 (CDN), `acm_Prompts.js` (bundled — set `ACM_APP_TITLE`), existing `lib/` files (`menu.css`, `menu.js`, `JWT-Tokens-Client.js`, `role-permissions.js`, `_config.js`)
