...
router.get('/help-editor/pages', async (req, res) => {
    ...
    try {
        ...
        const [rows] = await conn.execute('SELECT * FROM help_content ORDER BY page_key');
        res.json(rows);
    } catch (e) { console.error('help-editor/pages error:', e.message); res.status(500).json({ error: e.message }); }
...
router.post('/help-editor/page', async (req, res) => {
    ...
    try {
        ...
        const [result] = await conn.execute(
            'INSERT INTO help_content (page_key) VALUES (?)',
            [req.body.page_key]
        );
...
router.put('/help-editor/page/:id', async (req, res) => {
    ...
    try {
        conn = await mysql.createConnection(getDbConfig());
        await conn.execute(
            'UPDATE help_content SET page_key = ? WHERE help_content_id = ?',
            [req.body.page_key, req.params.id]
        );
