...
const HELP_API_BASE_PATH = '/api';        // ← Change if needed
const HELP_VIDEO_PORT = null;             // ← Set to port number (e.g. 55170) if videos are on a different port; null = same origin
...
async function loadConfig() {
    try {
        if (typeof _CONFIG !== 'undefined' && _CONFIG.SERVER_API_URL) {
            API_BASE = _CONFIG.SERVER_API_URL.replace('/api2', HELP_API_BASE_PATH); return;
        }
        let retries = 0;
        while (retries < 5 && (typeof _CONFIG === 'undefined' || !_CONFIG.SERVER_API_URL)) {
            await new Promise(r => setTimeout(r, 100)); retries++;
        }
        API_BASE = (typeof _CONFIG !== 'undefined' && _CONFIG.SERVER_API_URL)
            ? _CONFIG.SERVER_API_URL.replace('/api2', HELP_API_BASE_PATH)
            : `${window.location.protocol}//${window.location.hostname}:${window.location.port || 80}${HELP_API_BASE_PATH}`;
    } catch { API_BASE = `${window.location.protocol}//${window.location.hostname}:${window.location.port || 80}${HELP_API_BASE_PATH}`; }
}
