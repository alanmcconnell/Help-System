...
const HELP_VIDEO_PORT = null;             // ← e.g. 55170 if videos served on separate port; null = same as API
const HELP_API_URL_OVERRIDE = null;       // ← e.g. 'http://localhost:55170/api'; null = auto-detect
...
function _helpGetApiBase() {
    ...
    if (typeof _CONFIG !== 'undefined' && _CONFIG.SERVER_API_URL)
        return _CONFIG.SERVER_API_URL.replace('/api2', '/api');
    return `${window.location.protocol}//${window.location.hostname}:${window.location.port || 80}/api`;
}
